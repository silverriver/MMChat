...
class Trainer:
    ...
    def _eval_train(self, epoch):
        ...
        for i, data in ITER:
            ...
            outputs = self.model.decode(prevs, enc_contexts, img_feat=img_feat)
    ...
    def _eval_test(self):
        ...
        with torch.no_grad():
            ...
            for i, data in ITER:
                ...
                # blue, dist, f1
                preds, lens = self.model.top_k_top_p_search(enc_contexts, img_feat=img_feat, top_p=self.config.top_p, top_k=self.config.top_k, sample_size=1)
                for j in range(bs):
    ...
    def _pred_sample(self, n_sample):
        with torch.no_grad():
            ...
            enc_contexts = []
            enc_contexts.append(self.model.encode(context, context_seg_id))
            prediction = self.model.beam_search(enc_contexts, img_feat=img_feat)
