...
parser = argparse.ArgumentParser()
parser.add_argument('--config', help='config file',           default='dialog_image/infer_config.json')
parser.add_argument('--test_dialog_data', help='config file', default='data/dialog_test.json')
parser.add_argument('--test_img_index', help='config file',   default='data/weibo_img_index_test.json')
parser.add_argument('--img_feat', help='config file',         default='data/weibo_img/image_feature_lmdb')
...
try:
    ...
    test_dataset = dataset.DialogDataset(
        [args.test_dialog_data], [args.test_img_index], args.img_feat, vocab, logger, config.max_context_len, 
        config.max_resp_len, feat_num=config.img_feat_num)
    ...
    with torch.no_grad():
        ...
        with open(os.path.join(eval_dir, latest_ckpt + os.path.basename(args.out_file) + str(args.local_rank)), 'w', encoding='utf-8') as f:
            ...
            for data in ITER:
                ...
                img_feat = data['img_feat'].to(device, non_blocking=True)

                prediction = model.predict_beam([(context, context_seg_id)], img_feat=img_feat)
                for i in range(bs):
