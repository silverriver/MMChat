...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, padding_mask, masked_selfatt=None):
        ...
        if masked_selfatt is not None:
            apply_future_mask = masked_selfatt
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, padding_mask, masked_selfatt, *contexts):
        ...
        for i in range(0, len(inputs), 2):
            ...
            a = self.attn(x, c, c, m, masked_selfatt=masked_selfatt)
            full_attn += (a / n_attn)
...
class TransformerModule(nn.Module):
    def __init__(self, config, n_embeddings, padding_idx):
        super(TransformerModule, self).__init__()

        self.embeddings = nn.Embedding(n_embeddings, config.embeddings_size, padding_idx=padding_idx)
        self.pos_embeddings = nn.Embedding(config.n_pos_embeddings + 1, config.embeddings_size, padding_idx=0)
        self.embed_dropout = nn.Dropout(config.embed_dropout)
        self.layers = nn.ModuleList(
            [TransformerBlock(config.embeddings_size, config.n_heads, config.dropout, config.attn_dropout, config.ff_dropout) for _ in range(config.n_layers)])
        self.img_feat_proj = nn.Linear(2048, config.embeddings_size)
        self.config = config
    ...
    def forward(self, x, enc_contexts=[], x_seg_id=None, masked_selfatt=None):
        '''contexts = [(context1, padding_mask1), ...]'''
        ...
        if x_seg_id is not None:
            x = x + self.embeddings(x_seg_id) * math.sqrt(self.embeddings.embedding_dim)

        # if img_feat is not None:
        #     img_feat = torch.max(img_feat, dim=1, keepdim=True)[0]
        #     x = x + self.img_feat_proj(img_feat) * math.sqrt(self.embeddings.embedding_dim)

        x = self.embed_dropout(x)
        enc_contexts = sum(enc_contexts, ())

        # if img_feat is not None:
        #     enc_contexts += (self.img_feat_proj(img_feat) * self.config.img_feat_weight, torch.zeros(img_feat.shape[:2], device=img_feat.device))

        for layer in self.layers:
            out = layer(x, padding_mask, masked_selfatt, *enc_contexts)
            x = out[0]
