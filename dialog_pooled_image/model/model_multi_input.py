...
class MultiInputModel(nn.Module):
    ...
    def encode(self, x, x_seg_id=None, masked_selfatt=None, img_feat=None):
        enc, pad_mask = self.transformer_module(x, x_seg_id=x_seg_id, masked_selfatt=masked_selfatt)
        if img_feat is not None:
            img_feat = torch.max(img_feat, dim=1, keepdim=True)[0]
            if hasattr(self.transformer_module, 'img_feat_proj'):
                enc = enc + self.transformer_module.img_feat_proj(img_feat) * self.config.img_feat_weight
            else:
                enc = enc + self.transformer_module.module.img_feat_proj(img_feat) * self.config.img_feat_weight
        return enc, pad_mask
    ...
    def predict(self, contexts=[], img_feat=None):
        enc_contexts = [self.encode(c, c_seg_id, img_feat=img_feat) for c, c_seg_id in contexts]
