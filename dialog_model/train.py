...
parser.add_argument('--config', help='config file', default='dialog_model/config.json')
...
try:
    ...
    train_dataset = dataset.DialogDataset([config.train_data], vocab, logger, config.max_context_len, config.max_resp_len)
    valid_dataset = dataset.DialogDataset([config.valid_data], vocab, logger, config.max_context_len, config.max_resp_len)
    ...
    if latest_ckpt is None:  # start from scratch
        ...
        cgpt_model['embeddings.weight'] = torch.cat(
            [cgpt_model['embeddings.weight'], (torch.randn((len(vocab.spec_tokens) - len(vocab.pretrained_spec_tokens), config.embeddings_size), dtype=torch.float32) * 0.01).to(device)], 
            dim=0)
